...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      userChange(){
        // this.$store.dispatch('allUserSelect',this.user_id)
        this.fetchPosts();
      },
      billtypeChange(){
        this.fetchPosts();
      ...
      customerReportExport(){
        location.href = '/manager/report/customer/export?user_id='+this.user_id+'&receive_type='+this.receive_type+'&bill_type='+this.bill_type+'&start_date='+moment(this.date.start).format('YYYY-MM-DD')+'&end_date='+moment(this.date.end).format('YYYY-MM-DD');
      }
    }
  }
</script>
