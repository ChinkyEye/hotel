                  ...
                  <div class="col-md-3 border-right">
                    <form role="form" enctype="multipart/form-data" @submit.prevent="addNewItem()">
                      <div class="form-group">
                        ...
                        <a href="javascript:void(0)" class="float-right" data-toggle="modal" data-target="#addNew" @click="openModalWindow">Add New Category</a>
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="price" placeholder="Add Rate" v-model="form.price" name="price" :class="{ 'is-invalid': form.errors.has('price') }" autocomplete="off">
                        <has-error :form="form" field="price"></has-error>
                      </div>
                      <!-- <div class="form-group">
                        <label for="quantity">Quantity</label>
                        <input type="text" class="form-control" id="quantity" placeholder="Add Quantity" v-model="form.quantity" name="quantity" :class="{ 'is-invalid': form.errors.has('quantity') }" autocomplete="off" readonly="true">
                        <has-error :form="form" field="quantity"></has-error>
                      </div> -->
                      <div class="form-group">
                        <label for="unit_id">Unit</label>
                        <a href="javascript:void(0)" class="float-right" data-toggle="modal" data-target="#addNewunit" @click="openModalWindowUnit">Add New Unit</a>
                      ...
                      <div class="form-group">
                        ...
                        <input type="radio" id="three" value="3" v-model="form.item_type_id">
                        <label for="three">Bar</label>
                        <has-error :form="form" field="item_type_id"></has-error>
                      </div>
                      <div class="form-group" v-if="form.item_type_id != '2'">
                        <input type="text" class="form-control" id="stock" placeholder="Add Stock" v-model="form.stock" name="stock" :class="{ 'is-invalid': form.errors.has('stock') }" autocomplete="off">
                       <!-- <input type="text" name="stock" v-model="stock"> -->
                      </div>
                      <!-- /.card-body -->
                      <button type="submit" class="btn btn-primary btn-block" :disabled="state.isSending">{{state.isSending ? "Loading..." : "Add Item"}}</button>
                    </form>
                  </div>
                  <div class="col-md-9">
                      ...
                      <table class="table table-sm table-bordered table-hover">
                        ...
                        <tbody class="text-center">
                          <tr v-for="(item,index) in getAllItem" :key="item.id" :class="customerStatus(item.is_active)">
                            ...
                            <td>
                              <router-link :to="`/item/${item.id}/addimage`"><i class="fas fa-plus"></i></router-link> 
                              |
...
<script>
  import pagination from '../../../components/PaginationComponent.vue';
    ...
    addNewItem(){
      ...
      addNewCategory(){
        var that = this;
          ...
          this.form_add.post('/manager/category')
          .then((response)=>{
      ...
      addNewUnit(){
        var that = this;
          ...
          this.form_unit.post('/manager/unit')
          .then((response)=>{
    ...
    categoryChange(){
      ...
      salesReportExport(){
        location.href = '/manager/item/export?search='+this.search+'&category_id='+this.category_idSearch;
      }
		},
