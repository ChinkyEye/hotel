  ...
  <div>
    <div class="content-header">
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Edit Income</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
                  ...
                  <form role="form" enctype="multipart/form-data" @submit.prevent="addNewIncomeTopic()">
                    <div class="card-body">
                      <div class="form-group">
                        <label for="topic_id">Income Topic <code>*</code></label>
                        <select class="form-control" id="topic_id" v-model="form.topic_id" name="topic_id" :class="{ 'is-invalid': form.errors.has('topic_id') }"> 
                          ...
                          <option :value="incometopic.id" v-for="incometopic in getIncomeTopicSelect">{{incometopic.name}}</option>
                        </select>
                    ...
                    <div class="card-footer">
                      <button type="submit" class="btn btn-primary" :disabled="state.isSending">{{state.isSending ? "Loading..." : "Update Income"}}</button>
                    </div>
                  </form>
                </div>
              </div>
  ...
  import moment from 'moment'
    ...
    mounted(){
        // this.$store.dispatch("allUnittypeSelect")
         this.$store.dispatch("allIncomeTopicSelect")
    },
    created(){
          axios.get(`/manager/income/${this.$route.params.incomeid}/edit`)
            .then((response)=>{
              this.form.fill(response.data.units)
            })
        },
    computed:{
        getIncomeTopicSelect(){
          var a = this.$store.getters.getIncomeTopicSelect
          return a;
        },
    },
    methods:{
         addNewIncomeTopic(){
          ...
          this.form.put(`/manager/income/${this.$route.params.incomeid}`)
          .then((response)=>{
