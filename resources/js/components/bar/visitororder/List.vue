                            ...
                            <tr v-for="(order,index) in orders" :key="order.id">
                              ...
                              <td>
                                ...
                                <td>
                                  ...
                                  <td class="pt-3"><span v-on:click="remove(index)"><i class="fas fa-times-circle"></i></span></td>
                                </tr>
                              </tbody>
                              <tfoot>
                                <tr>
                                ...
                                <tr>
                                  <td colspan="5"></td>
                                  <td>
                                    <button type="submit" class="btn btn-success btn-block" :disabled="state.isOrdering">{{state.isOrdering ? "Please wait..." : "Save Order"}}</button>
                                  </td>
                                  <td></td>
                                </tr>
                              </tfoot>
...
<script>
    ...
    data(){
      ...
      this.fetchPosts();
        ...
        var that = this;
        this.$Progress.start()
				var avar = this.$store.getters.getItemSelectReady;
      ...
      appendList(){
        ...
        this.$Progress.start()
          axios.get('/bar/order/'+this.form.item_id)
          .then((response)=>{
              Toast.fire({
                  icon: 'success',
      ...
      fetchPosts() {
        this.$store.dispatch("allItemSelectReady")
        this.$store.dispatch("allTableSelect")
      },
    addNewItem(){
      this.state.isSending = true;
          this.form.post('/bar/item')
          .then((response)=>{
              Toast.fire({
                  icon: 'success',
