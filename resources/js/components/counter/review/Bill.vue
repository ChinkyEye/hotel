...
<script>
  ...
  export default{
    ...
    mounted(){
      this.$store.dispatch("allReviewBill" ,this.$route.params.billid)
    },
    computed:{
      getAllBill(){
        var data = this.$store.getters.getReviewBill;
        ...
        if((typeof data.orderstatus)!="undefined" && data.orderstatus.length == 0){
