      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <section class="col-lg-12 connectedSortable">
            <!-- main page load here-->
            <router-link class="btn btn-primary rounded-0" to="/order" style="color:#fff"> Add Order <i class="fas fa-user-plus fa-fw"></i></router-link>
            <!-- <router-link  class="btn btn-primary rounded-0"  to="/visitor-order" style="color:#fff"> Add Visitor Order <i class="fas fa-user-plus fa-fw"></i></router-link> -->
                  ...
                  <table class="table table-sm table-bordered table-hover">
                    ...
                    <tbody class="text-center">
                      <tr v-for="(orderdetail,index) in getAllOrderdetail" :key="orderdetail.id"  :class="customerStatus(orderdetail.is_active)">
                        ...
                        <td v-if="orderdetail.is_active == '0'">Removed </td>
                        <td v-else>
                          <a href="javascript:void(0)" @click.prevent="userShow(orderdetail.id)" title="Remove"><i class="nav-icon fas fa-times-circle"></i></a> Active
                        </td>
                        <td><span class="badge badge-warning"><span class="text-info">{{orderdetail.created_at  | formatDate}}</span></span></td>
                        <td><span><span class="text-danger">{{orderdetail.created_user.name}}</span></span></td>
                        <td v-if="orderdetail.is_active == '0'">
                          <router-link :to="`/confirm/${orderdetail.bill_id}/preview`"><i class="fas fa-eye" title="Click to show"></i></router-link>
                        </td>
                        <td v-else>
                          <router-link :to="`/confirm/${orderdetail.id}/show`"><i class="fas fa-eye" title="Click to show"></i></router-link>
                        </td>
                        <td>
                          <router-link :to="`/order/${orderdetail.bill_id}/preview/bar`">
                            <i class="far fa-folder" title="Click to view all BOT"></i>
                          </router-link> 
                          | 
...
<script>
  ...
  export default{
      ...
      userShow(clkid){
        ...
        Swal.fire({
            ...
            if(isConfirm.value === true) {
                axios.get('/counter/confirm/status/'+clkid)
                .then((response)=>{
                    Toast.fire({
                      icon: 'success',
    ...
    // userShow(clkid, show){
    //     axios.get('/counter/confirm/status/'+clkid+'/'+show)
    //     .then(()=>{
