  ...
  <div>
    <div class="content-header">
      <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6">
                <p class="text-danger m-0">Selling Category List</p>
            </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
          ...
          <section class="col-lg-12 connectedSortable">
            ...
            <div class="card">
              <div class="card-header">
                <div class="row">
                  <div class="col-md-2">
                    <router-link to="/category/selling/create"  class="btn btn-flat btn-danger btn-block" style="color:#fff"> Add Category <i class="fas fa-plus fa-fw"></i></router-link>
                  </div>
                  <div class="col-md-10">
                  ...
                  <table class="table table-sm table-bordered table-hover">
                    ...
                    <tbody>
                      <tr v-for="(category,index) in getAllCategory" :key="category.id"  :class="customerStatus(category.is_active)">
                        ...
                        <td>
                          <div class="btn-group">
                            <router-link :to="`/category/selling/${category.id}/edit`" class="btn btn-sm btn-flat btn-outline-primary"><i class="fas fa-pencil-alt" title="Click to edit"></i></router-link> 
...
<script>
  ...
  export default{
    ...
    computed:{
      getAllCategory(){
        this.$Progress.start()
        var avar = this.$store.getters.getSellingCategory;
        if(avar.length==2)
    ...
    methods:{
      fetchPosts() {
        this.$store.dispatch("allSellingCategory",[this.pagination.current_page,0]);
      },
      customerStatus(id){
        return {
            'table-danger':!id,
      ...
      deleteCategory(id){
        ...
        Swal.fire({
            ...
            if(isConfirm.value === true) {
                axios.delete('/home/category/selling/'+id)
                .then((response)=>{
                    Toast.fire({
                      icon: 'success',
                      title: 'Category Deleted successfully'
                  })
                    that.$store.dispatch("allSellingCategory", [0,0]);
                })
                .catch((response)=>{
                    Toast.fire({
                      icon: 'error',
    ...
    userShow(clkid, show){
        axios.get('/home/category/selling/status/'+clkid+'/'+show)
        .then(()=>{
            this.$store.dispatch("allSellingCategory", [0,0]);
            Toast.fire({
                icon: 'success',
    ...
    pagechange(){
      this.$store.dispatch("allSellingCategory", [this.pagination.current_page,this.search]);
    },
    searchCategory(){
      this.pagechange();
    }
