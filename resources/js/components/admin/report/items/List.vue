                ...
                <div class="row">
                  ...
                  <div class="form-group col-md pb-0">
                    <select class="form-control" id="manager_id" v-model="manager_id"  @change="managerChange"> 
                      <option value="">Select All Manager</option>
                      <option :value="manager.id" v-for="manager in getAllManager">{{manager.name}}</option>
                    </select>
                  </div>
...
<script>
  ...
  export default{
    ...
    pagechange(){
      this.$store.dispatch("allConfirmOrder", [this.pagination.current_page, this.category_id, this.item_id,this.manager_id,moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD')]);
    },
    categoryChange(){
      this.pagechange();
    },
