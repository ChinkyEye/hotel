    ...
    <table class="table table-sm table-bordered table-hover">
      ...
      <tbody>
        <tr v-for="(orderdetail,index) in getAllOrderdetail" :key="orderdetail.id"  :class="customerStatus(orderdetail.is_active)">
          ...
          <td>
            <span v-if="orderdetail.bill_type == '1'">Due<i class="nav-icon fas fa-times-circle"></i></span> 
              
            <span v-else-if="orderdetail.bill_type == '0'">Paid<i class="nav-icon fas fa-check-circle"></i></span> 
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      pagechange(){
        this.$store.dispatch("allConfirmOrderdetail", [this.pagination.current_page,this.search,this.user_t_id,this.table_id,this.manager_id,this.bill_type,moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD')]);
      },
      searchOrderdetail(){
        this.pagechange();
      },
      ...
      salesReportExport(){
        location.href = '/home/report/sales/export?bill_id='+this.search+'&user_type='+this.user_t_id+'&table_id='+this.table_id+'&bill_type='+this.bill_type+'&start_date='+moment(this.date.start).format('YYYY-MM-DD')+'&end_date='+moment(this.date.end).format('YYYY-MM-DD');
      }
    }
  }
</script>
