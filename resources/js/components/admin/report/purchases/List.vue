...
<script>
  ...
  export default{
    ...
    pagechange(){
      ...
      this.$store.dispatch("allPurchaseOrderdetail", [this.pagination.current_page,this.buyer_id, this.search, this.category_id,this.manager_id,moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD')]);
    },
    managerChange(){
      this.pagechange();
    },
    ...
    purchaseReportExport(){
      location.href = '/home/report/purchases/export?buyer_id='+this.buyer_id+'&search='+this.search+'&category_id='+this.category_id+'&start_date='+moment(this.date.start).format('YYYY-MM-DD')+'&end_date='+moment(this.date.end).format('YYYY-MM-DD');
