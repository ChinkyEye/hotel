  ...
  <div>
    <div class="content-header">
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Stocks Report</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            ...
            <div class="card">
              <div class="card-header">
                <div class="row">
                  ...
                  <div class="form-group col-md">
                    <select class="form-control" id="manager_id" v-model="manager_id" @change="managerChange"> 
                      <option value="">Select Manager</option>
                      <option :value="manager.id" v-for="manager in getAllManagerSelect">{{manager.name}}</option>
                    </select>
                  </div>
                ...
                <div id="printMe">
                  <div class="table-responsive">
                    <table class="table table-sm table-bordered table-hover">
                      ...
                      <tbody>
                        <tr v-for="(stock,index) in getAllOrder" :key="stock.id" :class="stock.qty_rem == stock.stock ? 'btn-danger' : 'btn-primary'">
                          <td>{{index+1}}</td>
                          ...
                          <td v-if="stock.item_type_id == '1'"> Ready Made </td>
                          <td v-else-if="stock.item_type_id == '2'"> Kitchen </td>
                          <td v-else>Bar</td>
...
<script>
  ...
  export default{
    ...
    computed:{
      ...
      getAllOrder(){
        this.$Progress.start()
        var bvar = this.$store.getters.getStockdetail;
        if(bvar.length == 0) return [];
        var avar = bvar[0].stockdetails.data;
        console.log(avar);
      ...
      getAllManagerSelect(){
          this.$Progress.finish()
          return this.$store.getters.getManagerSelect
      },
    ...
    methods:{
      fetchPosts() {
        ...
        this.$store.dispatch("allManagerSelect")
    ...
    pagechange(){
      this.$store.dispatch("allStockdetail", [this.pagination.current_page, this.search, this.category_id,this.manager_id,this.type,moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD')]);
    },
    categoryChange(){
      this.pagechange();
    },
    ...
    itemReportExport(){
      location.href = '/home/report/stocks/export?search='+this.search+'&category_id='+this.category_id+'&type='+this.type+'&start_date='+moment(this.date.start).format('YYYY-MM-DD')+'&end_date='+moment(this.date.end).format('YYYY-MM-DD');
    }
    }
  }
</script>
