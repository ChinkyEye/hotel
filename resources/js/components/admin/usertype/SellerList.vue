                    ...
                    <section class="col-lg-12 connectedSortable">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-md-2">
                                         <router-link to="/customer/seller/create" class="btn btn-flat btn-danger btn-block" style="color:#fff"> Add Seller <i class="fas fa-plus fa-fw"></i></router-link>
                                    </div>
                                    <div class="col-md-10">
                                        <input type="text" class="form-control" @keyup="searchService" v-model="search" placeholder="Search Seller">
                                    </div>
                                </div>
                            </div><!-- /.card-header -->
                            <div class="card-body">
                                            ...
                                            <tr v-for="(post,index) in allusertypes" :key="post.id" :class="customerStatus(post.is_active)">
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <router-link :to="`/customer/${post.id}`" class="btn btn-sm btn-flat btn-outline-primary"><i class="fas fa-eye" title="Click to view"></i></router-link>
                                                    </div>
                                                </td>
                                            </tr>
                                        </tbody>
...
methods:{
    ...
    deleteUser(id){
        ...
        Swal.fire({
            ...
            if(isConfirm.value === true) {
                axios.get('/home/user-type/delete/'+id)
                .then((response)=>{
    ...
    userShow(clkid, show){
        axios.get('/home/customer/seller/status/'+clkid+'/'+show)
        .then(()=>{
            this.$store.dispatch("getAllSellerType", [this.pagination.current_page,0]);
            Toast.fire({
                icon: 'success',
    ...
    userSalary(clkid, show){
        axios.get('/home/customer/seller/salary/'+clkid+'/'+show)
        .then(()=>{
            this.$store.dispatch("getAllSellerType", [this.pagination.current_page,0]);
            Toast.fire({
                icon: 'success',
