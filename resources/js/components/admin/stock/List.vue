              ...
              <div class="card-body">
                <!-- <div class="col-md-3">
                  <form role="form" enctype="multipart/form-data" @submit.prevent="addNewStock()">
                    ...
                    <div class="form-group">
                      <label for="buyer_id">Bought User</label>
                    ...
                    <button type="submit" class="btn btn-primary" :disabled="state.isSending">{{state.isSending ? "Loading..." : "Add Stock"}}</button>
                  </form>
                </div> -->
                  ...
                  <table class="table table-sm table-bordered table-hover">
                    ...
                    <tbody>
                      <tr v-for="(stock,index) in getAllStock" :key="stock.id">
                        ...
                        <td>
                          <div class="btn-group">
                            <router-link :to="`/stock/${stock.id}/edit`" class="btn btn-sm btn-flat btn-outline-primary"><i class="fas fa-pencil-alt" title="Click to edit"></i></router-link> 
                            <a href="" @click.prevent="deleteStock(stock.id)" class="btn btn-sm btn-flat btn-outline-danger"><i class="fas fa-trash-alt" title="Click to delete"></i></a>
                          </div>
                        </td>
                      </tr>
                    </tbody>
...
<script>
  ...
  export default{
      ...
      deleteStock(id){
        ...
        Swal.fire({
            ...
            if(isConfirm.value === true) {
                axios.delete('/home/stock/'+id)
                .then((response)=>{
                    Toast.fire({
                      icon: 'success',
                      title: 'Stock Deleted successfully'
                  })
                that.$store.dispatch("allStock",[0,0,0,moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD')]);
                })
                .catch((response)=>{
                    Toast.fire({
                      icon: 'error',
    ...
    pagechange(){
      console.log(moment(this.date.start).format('YYYY-MM-D'));
      console.log(moment(this.date.end).format('YYYY-MM-D'));
      this.$store.dispatch("allStock", [this.pagination.current_page,this.search,this.category_idSearch, moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD')]);
    },
    searchStock(){
      this.pagechange();
    },
