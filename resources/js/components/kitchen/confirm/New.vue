                  ...
                  <table class="table table-sm table-bordered table-hover">
                    ...
                    <tbody class="text-center">
                      <tr v-for="(orderdetail,index) in getAllOrderdetail" :key="orderdetail.id"  :class="customerStatus(orderdetail.is_active)">
                        ...
                        <td v-else><i class="nav-icon fas fa-check-circle"></i>Confirmed</td>
                        <td v-if="orderdetail.is_active == '0'"><i class="nav-icon fas fa-times-circle"></i>Removed </td>
                        <td v-else>
                          <i class="nav-icon fas fa-check-circle"></i> Active
                        </td>
...
<script>
  ...
  export default{
      ...
      userShow(clkid){
        ...
        Swal.fire({
            ...
            if(isConfirm.value === true) {
                axios.get('/kitchen/confirm/status/'+clkid)
                .then((response)=>{
                    Toast.fire({
                      icon: 'success',
    ...
    // userShow(clkid, show){
    //     axios.get('/kitchen/confirm/status/'+clkid+'/'+show)
    //     .then(()=>{
