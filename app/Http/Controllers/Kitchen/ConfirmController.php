...
class ConfirmController extends Controller
{
    public function index(Request $request){
        ...
        $posts = $posts->with('customer','usertype','table')->withCount('count')->where('is_confirmed','0')->paginate(15);
        $response = [
           'pagination' => [
               'total' => $posts->total(),
    ...
    public function orderbill($bill){
        $bills = Order_detail::where('bill_id', $bill)->with('order_kitchen','customer','table','createdUser')->first();
        return response()->json([
            'bills'=>$bills
        ],200);
    }
