<?php

namespace App\Http\Controllers\Kitchen;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Table;
use App\Item;
use App\Order;
use App\Order_detail;
use App\User;
use App\Usertype;
use App\Bill_has_sn;
use Auth;
use Illuminate\Support\Facades\Hash;

class VisitorOrderController extends Controller
{
    public function store(Request $request)
    {   
        $bill_id = strtotime(date(("Y-m-d H:i:s")));
        $current_date = date("Y-m-d");
        $current_time = date("H:i:s");
        $auth = Auth::user()->id;

        $name = $request->name;
        $phone = $request->phone;
        $address = $request->address;

        $user_type = Usertype::where('level_id','3')->value('id');
        // var_dump($user_type);
        if($name == ''){
            $check_exist_blank = User::where('name','N-A')->where('type_id','2')->count();
            if($check_exist_blank){
                $uid = User::where('name','N-A')->where('type_id','2')->value('id');
            }
            else{
                $user = New User();
                $user->name = 'N-A';
                $user->user_code = strtotime("now");
                $user->email = rand().'na'.'@techware.com.np';
                $user->password = Hash::make(rand());
                $user->phone = 'N-A';
                $user->address = 'N-A';
                $user->is_active = '1';
                $user->user_type = '0';
                $user->type_id = '2';
                $user->user_type_id = $user_type;
                $user->date = $current_date;
                $user->date_np = $this->helper->date_np_con();
                $user->time = $current_time;
                $user->save();
                $uid = $user->id;
            }
        }
        else{
            $user = New User();
            $user->name = $request->name;
            $user->user_code = strtotime("now");
            $user->email = rand().'na'.'@techware.com.np';
            $user->password = Hash::make($phone?$phone:'na');
            $user->phone = $phone?$phone:'-';
            $user->address = $address?$address:'-';
            $user->is_active = '1';
            $user->user_type = '0';
            $user->type_id = '2';
            $user->user_type_id = $user_type;
            $user->date = $current_date;
            $user->date_np = $this->helper->date_np_con();
            $user->time = $current_time;
            $user->save();
            $uid = $user->id;
        }

        $calc_gtotal = 0;
        $cal_discount = 0;
        $req_disc = 0;
        $req_total = 0;
        $loop_check = $request->data;
        foreach ($loop_check as $key => $value) {
            // var_dump($value['quantity']); die();
            $calc_value = Item::where('id', $value['id'])->get();
            foreach ($calc_value as $index => $calc_values) {
                $calc_item_id = $calc_values->id;
                $calc_unit_id = $calc_values->unit_id;
                $calc_item_type_id = $calc_values->item_type_id;
                $calc_rate = $calc_values->price;
                $calc_cat_id = $calc_values->category_id;
                $calc_mg = 0;
                $calc_weight = 0;
                $calc_discount = 0;
            }
            $order = New Order();
            $order->customer_id = $uid;
            $order->usertype_id = $user_type;
            $order->bill_id = $bill_id;
            $order->item_id = $calc_item_id;
            $order->category_id = $calc_cat_id;
            $order->unit_id = $calc_unit_id;
            $order->quantity = $value['quantity'];
            $ca_discount = $value['quantity']*($calc_discount);
            $cal_tot = $value['quantity']*($calc_rate - $calc_discount);
            $order->total = $cal_tot;
            $order->date = $current_date;
            $order->date_np = $this->helper->date_np_con();
            $order->time = $current_time;
            $order->is_confirmed = '1';
            $order->bill_type = '0';
            $order->item_type_id = $calc_item_type_id;
            $order->created_by = $auth;
            $order->save();
        $calc_gtotal += $cal_tot;
        $cal_discount += $ca_discount;
        }
        $req_disc = $cal_discount + $request->mdiscount;
        $req_total = $calc_gtotal - $request->mdiscount;
        $order_total = new Order_detail;
        $order_total->bill_id = $bill_id;
        $order_total->total = $calc_gtotal;
        $order_total->discount = $req_disc;
        $order_total->grand_total = $req_total;
        $order_total->table_id = $request->table_id;
        $order_total->customer_id = $uid;
        $order_total->usertype_id = $user_type;
        $order_total->date = $current_date;
        $order_total->date_np = $this->helper->date_np_con();
        $order_total->time = $current_time;
        $order_total->received = $req_total;
        $order_total->is_active = '1';
        $order_total->is_confirmed = '1';
        $order_total->bill_type = '0';
        $order_total->created_by = $auth;
        $order_total->save();
        
        $bill_sn = new Bill_has_sn();
            $bill_count = Bill_has_sn::count();
            $bill_sn->bill_sn = $bill_count+1;
            $bill_print_count = Bill_has_sn::where('bill_id', $bill_id)->count();
            $bill_sn->is_print = 1;
            $bill_sn->print_count = $bill_print_count+1;
            $bill_sn->bill_id = $bill_id;
            $bill_sn->date = $current_date;
            $bill_sn->date_np = $this->helper->date_np_con();
            $bill_sn->time = $current_time;
            $bill_sn->created_by = $auth;
            $bill_sn->save();

        return response()->json([
            'message'=>'ok',
            'bill_id' => $bill_id
        ],200);
    }

    public function bill($bill){
        $bills = Order_detail::where('bill_id', $bill)->with('order','customer','table')->first();
        return response()->json([
            'bills'=>$bills
        ],200);
    }
}