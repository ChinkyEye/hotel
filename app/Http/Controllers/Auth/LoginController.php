    ...
    public function login(Request $request)
    {
        ...
        if(Auth::attempt(['email' => $request->email, 'password' => $request->password, 'is_active' => 1], true)) {
            ...
            elseif(Auth::user()->user_type == '3'){
                return Redirect::route('counter')->with('alert-success', 'Login success! Please enjoy!!');
            }
            elseif(Auth::user()->user_type == '5'){
                return Redirect::route('bar')->with('alert-success', 'Login success! Please enjoy!!');
            }
            elseif(Auth::user()->user_type == '6'){
                return Redirect::route('kitchen')->with('alert-success', 'Login success! Please enjoy!!');
            }
            else{
