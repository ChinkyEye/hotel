...
class ConfirmController extends Controller
{
    public function index(Request $request){
        ...
        // $posts = Order_detail::orderBy('id','DESC')->where('created_by', Auth::user()->id);
    ...
    {   
        ...
        foreach ($loop_check as $key => $value) {
            ...
            $cal_usertype_id = Order_detail::where('bill_id', $bill_id)->value('usertype_id');
            $getDefineDiscount = Usertype::where('id',$cal_usertype_id)->value('discount');
    ...
    public function orderbill($bill){
        $bills = Order_detail::where('bill_id', $bill)->with('order','customer','table','createdUser')->first();
        return response()->json([
            'bills'=>$bills
        ],200);
    }
