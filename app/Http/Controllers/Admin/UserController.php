...
class UserController extends Controller
{
    ...
    public function index(Request $request){
      ...
      $posts = User::orderBy('id','DESC')->where('user_type_id', $id)->with('usertype','getSalary','userpackage','getPack');
      if(empty($request->search))
        {            
          $posts = $posts;
        }
            ...
            $search = $request->search;
            $posts = $posts->where('name', 'LIKE',"%{$search}%")->where('user_type_id',$id)->orWhere('user_code', 'LIKE',"%{$search}%")->orWhere('phone', 'LIKE',"%{$search}%");
        }
        $posts = $posts->where('user_type','0')->where('user_type_id', $id)->paginate(15);
        $response = [
           'pagination' => [
               'total' => $posts->total(),
               'per_page' => $posts->perPage(),
