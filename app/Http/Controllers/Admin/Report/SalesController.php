...
use App\Exports\Admin\SaleReportExport;
...
class SalesController extends Controller
{
  public function index(Request $request){
    ...
    if($request->has('manager_id') && $request->get('manager_id')!=""){
          // var_dump($request->table_id);
      $posts = $posts->where('created_by',$request->manager_id);
    }
  ...
  public function fileExport(Request $request) 
    {
      ...
      return Excel::download(new SaleReportExport($request->bill_id, $request->user_type, $request->table_id, $request->bill_type, $request->start_date, $request->end_date), $filename);
    }
}