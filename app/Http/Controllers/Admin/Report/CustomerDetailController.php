...
use App\Exports\Admin\CustomerReportExport;
use Maatwebsite\Excel\Facades\Excel;

class CustomerDetailController extends Controller
{
  public function index(Request $request){
      $posts = Order_detail::orderBy('id','DESC')->where('is_confirmed','1')->where('customer_id',$request->user_id);
  ...
  public function store(Request $request)
    {   
      ...
      //   ]);
        ...
        if($configure->income_receiveamount == 1){
          ...
          $incometopic = IncomeExpenseTopic::firstOrCreate([
              ...
              'name' => 'Received Amount',
              'type' => '1',
              'slug' => strtolower('Received Amount'),
              'is_active' => '1',
              'date' => date("Y-m-d"),
          ...
          $incomeexpense_id = IncomeExpenseTopic::where('name','Received Amount')->value('id');
