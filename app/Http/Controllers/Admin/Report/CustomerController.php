...
class CustomerController extends Controller
{
  public function index(Request $request){
    ...
    $posts = Order_detail::orderBy('id','DESC')->where('receive_type','0')->where('is_confirmed','1')->where('customer_id',$request->user_id);
  ...
  public function pay(Request $request){
    $posts = Order_detail::orderBy('id','DESC')->where('receive_type','1')->where('bill_type','2')->where('is_confirmed','1')->where('customer_id',$request->user_id);
